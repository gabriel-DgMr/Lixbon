...
- **Pasarela (Stripe)**, tras revisar todos los flujos:
  ...
  - **Probado contra Stripe (test) desde el navegador** (alta de tarjeta con 3-DS, mejora, bajada, mejora con tarjeta que exige autenticación en cada cobro → pending_update → banco → /resolve → «Pago aprobado»). Destapó y se arreglaron: `change_plan` ignoraba la tarjeta elegida (la prorrata iba a la predeterminada anterior); `_cobro_de_factura` se detenía en `confirmation_secret` y la pantalla decía «$0.00» o enseñaba el cobro anterior en una bajada; y `_a_dict` usaba `to_dict_recursive`, que stripe-python ≥ 13 eliminó: **`sync_subscription` llevaba rota** (solo el webhook sincronizaba) y `/resolve` devolvía 503. El 503 de la pasarela ahora incluye `error` (tipo y texto) para diagnosticar.
  - Pendiente en Stripe (no en código): la cuenta en modo test tiene cobros y depósitos «bloqueados» hasta completar la activación; el PaymentElement muestra una pestaña «Banco» (Link) que conviene apagar en Dashboard → Payment methods, porque la suscripción solo admite tarjeta.
  - Tests: `core/billing/test_stripe_webhook_guards.py`.
- **Posicionamiento (las cuatro palancas)**:
  1. **Portada** `pages/LandingPage.jsx` siempre en `/` (el chat vive en `/chat`, noindex; con sesión los botones dicen «Ir al chat»). Minimalista, sin tarjetas: una columna de 1080 px, titulares a peso 500, secciones numeradas con reglas finas, productos en lista, precios sobre regla, FAQ en dos columnas. Dos ilustraciones de línea en `components/IlustracionesLanding.jsx` (clúster y recorrido del mensaje). **Tema claro/oscuro global**: `:root[data-theme="light"]` en `base.css` redefine todos los tokens (más `--primary-hover`, `--code-inline`, `--sombra-menu`); `hooks/useTema.js` (estado compartido con `useSyncExternalStore`, `localStorage` `lixbon-tema`, `data-theme` en `<html>`; `index.html` lo aplica antes del primer pintado) y `components/TemaBoton.jsx` (sol/luna) en PublicNav, Visuals (galería y editor), Ajustes, Admin, compartidos, Remote y el menú de perfil del chat. Las hojas de estilo dejaron de usar hex fijos para superficies y tintas (barrido a tokens). Por defecto oscuro. H1, razones, cuatro productos, ejemplo de API, precios resumidos y FAQ con esquemas `Organization`, `WebSite` y `FAQPage`. Estilos en `styles/landing.css`.
  2. **Guías** `/guias` y `/guias/:slug` (`guiasIndex.js` + `guiasContent.jsx`, esquema `Article`): API compatible con OpenAI, Qwen 3.5 desde Python, agente de código en la terminal, IA con datos privados. Enlazadas en la navegación pública y el pie.
  3. **Perfiles de la marca**: `PERFILES` en `lib/seo.js` (vacío) → `sameAs` del `Organization`. Rellenar con GitHub/LinkedIn/X reales; el alta en directorios y Product Hunt es manual.
  4. **Prerender**: `npm run build` = `vite build` + `vite build --ssr src/entry-server.jsx` + `scripts/prerender.mjs`, que renderiza con React en Node cada ruta de `src/rutasPublicas.js` (la misma lista del sitemap) y escribe `dist/_prerender/<ruta>/index.html` con el contenido y las etiquetas SEO en el `<head>` (`useSeo` las deja en `tomarSeoDelRender()` cuando no hay `window`). El gateway (`serve_frontend`) sirve ese HTML si la petición **no trae cookie de sesión**; con sesión, el shell de la SPA. Las páginas que renderizan en servidor no pueden tocar `window` en el render (DocsPage, DownloadsPage, VisualsPage ya guardan el acceso).
- **Legal** (`/legal/privacidad`, `/legal/terminos`, `/legal/reembolsos`): `pages/legalContent.jsx` (índice sin JSX en `legalIndex.js` para el sitemap), `LegalPage.jsx`, pie `PublicFooter` en docs/planes/aplicaciones/novedades, y consentimiento en el registro. Los datos del responsable (`RESPONSABLE` en `legalContent.jsx`) están entre corchetes: rellenar nombre/NIT/dirección antes de cobrar en real. Privacidad redactada sobre la Ley 1581/2012 y lo que la plataforma hace de verdad (Railway, Stripe, nodos GPU, Cloudflare).
- **Sin tarifa comodín pública**: cobrar un modelo sin precio publicado no es legal. `resolve_pricing` responde `403 model_not_priced` si no hay fila (ni `*` activo); el precheck solo exige tarifa cuando se va a cobrar por créditos (los planes Pro/Advance usan cuota). `/api/pricing` no lista `*`; el seed lo siembra inactivo y en producción quedó desactivado. Tests: `core/billing/test_credits_pricing.py`.
- **Tarifas** (panel → Tarifas, aplicadas en producción): `qwen3.5:27b` (Lixbon 1) 0.40/1.20, `qwen3.5:122b` 0.60/2.00, `gpt-oss:120b` 0.50/1.80, `qwen3:` 0.08/0.25, `deepseek-r1:` 0.10/0.30, `*` 0.30/0.90 ($/Mtok entrada/salida). Base: RTX 4090 ≈ $0.45/h → ~$1.25/M de salida al 100 % de uso; A100 80 GB ≈ $1.30/h → ~$1.80/M. `/api/pricing` enseña el alias público (`lixbon-1`).
...
- **Ediciones en Visuals**: un cambio sobre una página existente se pide como bloque `edit:nombre` con pares `<<<<<<< SEARCH / ======= / >>>>>>> REPLACE` (varios por bloque); `extraerEdiciones`/`aplicarEdiciones` en `lib/visuals.js` y `extract_edits`/`apply_edits` en `core/inference/visual_files.py` los aplican sobre la versión anterior (comparación línea a línea sin espacios sobrantes). Si un SEARCH no encaja, el chat lo avisa con «Pedir el archivo completo» y la página se queda como estaba. Los archivos nuevos siguen yendo completos en `file:nombre`.
- **Saneado de páginas**: `sanearHtml` (web) y `sanear_html` (gateway) corrigen al extraer dos fallos deterministas vistos en gpt-oss: `@apply` dentro de un `<style>` normal con Tailwind CDN (se le pone `type="text/tailwindcss"`) y una regla `.focus-visible {}` en vez de `:focus-visible` (recuadro permanente en cada enlace). El prompt de Visuals además los prohíbe.
- **Niveles de razonamiento**: `think` acepta `bool` o `low|medium|high`. `think_param()` (`core/inference/ollama.py`) adapta por modelo: gpt-oss no admite apagarlo (False→`low`, True→`medium`, niveles tal cual); el resto solo entiende booleanos (nivel→True). Títulos y JSON van con False; Visuals manda False (qwen metía el HTML en el thinking) y el gateway lo sube a `medium` si el modelo es gpt-oss; el CLI manda `high` en agent/plan y nada en ask.
...
- **Herramientas del CLI (sep 2026)**: `read_file` lee **PDF** (texto por páginas, pypdf) y **.docx** (python-docx, ambos autoinstalables), avisa en binarios y **adjunta las imágenes al modelo** como visión en el mensaje siguiente (`session["tool_images"]`). `edit_file` acepta `old_text` que difiera solo en espacios finales o indentación (reindenta `new_text`), conserva CRLF y dice la línea. `search` es literal por defecto (`regex`, `ignore_case`, `glob`), ignora `node_modules`/`.git`… y corta a 300 líneas; `find_files(pattern)` glob por nombre/ruta; `list_files` con tamaños y `recursive`. `run_command` mata el árbol de procesos al agotar el timeout (tope 600 s) y decodifica cp850/utf-8. Nuevas: `fetch_url` (html→texto, 12k chars) y `web_search` (endpoint `POST /api/websearch` del gateway, mismos proveedores que el modo investigar). Módulo `lixbon_cli/documents.py`; tests en `apps/cli/tests/test_tools.py` y `test_documents.py`. El CLI ya usa todo el ancho de la terminal (sin tope de 100 columnas).
- **Visuals (sep 2026)**: `/visuals` es la **galería** (miniaturas de la última versión vía `GET /api/conversations/{id}/files`, última edición, autor, renombrar/eliminar) y `/visuals/:id` el **editor** al estilo Claude Design: barra superior con volver, chat plegable (icono de panel), historial de versiones, título con desplegable de páginas (Lienzo / cada página / renombrar), design system, Seleccionar, Código, Presentar y **Compartir** (enlace público `/s/<token>` que renderiza el diseño, «Enviar a Lixbon CLI» = `/visual <id> [stack]` que descarga las páginas al workspace y, con un menú o `react`, `react api` o un stack libre, pone al agente a replicarlas como proyecto en `<slug>-app/` (`visual_project_prompt` en commands.py), Descargar HTML o .zip, copiar código). Sin sidebar: los chats de Visuals y de la web son independientes (`source`). `core/inference/visual_files.py` es el parser en Python. El modelo entrega `file:index.html`/`.svg` con `VISUALS_PROMPT` (+ **design system** elegido: presets en `DESIGN_SYSTEMS` o uno propio, guardado en localStorage por conversación); **varias páginas** por respuesta (`extraerArchivos` acepta el nombre en el lenguaje del bloque, en la primera línea de dentro, en la línea anterior, o lo deduce del `<title>`; las no reescritas se heredan de la versión anterior), lienzo libre `Board` (artboards, arrastrar, Ctrl+rueda), chat/lienzo ocultables, sin selector de dispositivo (la responsividad la hace el modelo), tarjeta «Escribiendo x.html…» en vez de código y overlay de carga del iframe, vista *Todas* con artboards escalados y navegación entre páginas por `<a href>` (script `INSPECTOR` inyectado: `lixbon:navigate`); **edición en el lienzo** (*Seleccionar* → `lixbon:select` → panel `Inspector` con texto/estilos → ops `{selector,text,style}` aplicadas en vivo y horneadas con `aplicarOps` (DOMParser) al copiar/descargar; guardadas en localStorage; *Pedir un cambio al modelo* rellena el prompt con el elemento). Iframe `sandbox` con Tailwind CDN; versiones, móvil/tablet/escritorio, código, copiar, abrir, descargar; conversaciones con `source=visuals`. **Imágenes**: nodo con `LIXBON_IMAGE_MODEL` (diffusers, agente 4.1: `/images/generate`, carga perezosa y descarga de VRAM tras `LIXBON_IMAGE_IDLE_S`; `LIXBON_IMAGE_OFFLOAD=1` para GPUs justas) → gateway `GET /api/images/status`, `POST /api/images/generate` (`routers/images.py`; elige nodo por score, guarda prompt e imagen como Markdown en la conversación; cuota del plan con `IMAGE_TOKEN_EQUIV`, sin tarifa propia aún). Tests: `core/gateway/test_images_endpoint.py`.
- **Alias públicos de modelo (sep 2026)**: tabla `model_aliases` (`lixbon-1` / «Lixbon 1» → `qwen3.5:27b`), panel *Admin → IA → Alias* (`/api/admin/model-aliases`). `core/inference/aliases.py`: `to_real` en `model_for_request` (y `key_model`), `public_catalog` en `/v1/models` (los usuarios solo ven modelos con alias si hay alguno; los admin ven todo), `to_public` en roles y respuestas. Web y CLI muestran `name`; tarifas y roles siguen sobre el modelo real.
- **Archivos desde el chat web (sep 2026)**: el modelo escribe un bloque ```file:nombre.ext``` (instrucción fija `FILE_PROMPT` que el web añade como system en `stream.js`); `Markdown.jsx` lo pinta como `FileCard` (descargar + ver contenido) y `lib/archivos.js` construye el archivo en el navegador: `.docx` desde Markdown (`docx`, títulos/listas/negrita/tablas), `.xlsx` desde tablas Markdown o CSV con `## Hoja: nombre` y fórmulas (SheetJS; chunks perezosos), el resto tal cual. Los bloques de código normales llevan copiar y descargar. Sin PDF ni pptx aún.
- **CLI como agente de código (sep 2026)**: compactación automática del contexto (>70 % de la ventana → resumen con el modelo, `context.compact_messages`; `/compact` manual), `run_command(background)` + `read_output` + `stop_command` (+ `/ps`), verificación tras editar (`checks.py`: ruff/py_compile, eslint/node --check, json; `/check`), checkpoints por turno y `/undo`, `/plan` (solo lectura + plan numerado), tools `todo`, `ask_user`, `outline`, `multi_edit`, `insert_at_line`, permisos por prefijo de comando (`/allow`, `allowed_commands` en config.json; un comando encadenado necesita todos sus tramos permitidos), `/commit` con mensaje del modelo, comandos propios (`.lixbon/commands/*.md` → `/nombre`, `$ARGUMENTS`) y **cliente MCP stdio** (`mcp.py`, `.lixbon/mcp.json` o `~/.lixbon/mcp.json`; tools expuestas como `mcp__servidor__tool`, aprobación como escritura; `/mcp`). Tests: `test_devtools.py`, `test_plan_tools.py`, `test_edit_tools.py`, `test_custom_commands.py`, `test_mcp.py`.
- **Búsqueda web `auto` (sep 2026)**: `web_search` acepta `true | "auto" | false`. En auto el planificador puede devolver `{"queries": []}` (no hace falta internet) y no se busca; con `tools` en la petición (agente) se salta el planificador porque el modelo tiene `web_search` como herramienta. Web: sin el globo manda `auto`, con el globo `true`. CLI: `/web auto|on|off` (default auto).
- **Nodo Windows (sep 2026)**: `run.ps1` es un supervisor (relanza el agente cada 15 s si muere; para solo con código 3) y la tarea de logon lleva 20 s de retraso y permisos de batería. Motivo: tras reiniciar, el arranque a los 10 s del logon moría sin escribir nada (0xC000013A; alias de Python de la Store/red/Ollama aún no listos). El instalador sin `LIXBON_ENROLL` pero con `node.json` actúa como actualización (conserva identidad y node.env).
...
- **Formas de arrancar un nodo (sin clonar el repo)**: (1) Linux: `curl -fsSL https://lixbon.com/install-node.sh | LIXBON_ENROLL=<token> bash` (instala Ollama y deja servicio systemd o proceso en background); (1b) Windows: `$env:LIXBON_ENROLL="<token>"; irm https://lixbon.com/install-node.ps1 | iex` (descarga `agent.py` a `%LOCALAPPDATA%\Lixbon
ode`, instala Python/Ollama con winget si faltan y registra tarea de usuario al iniciar sesión con `Register-ScheduledTask`, sin admin — `schtasks /SC ONLOGON` exige admin); (2) imagen Docker `infra/node/Dockerfile` (Ollama + agente, variables `LIXBON_GATEWAY`, `LIXBON_ENROLL`, `LIXBON_MODELS`); (3) con el repo: `LIXBON_ENROLL=<token> python -m core.node_agent.agent --connect https://lixbon.com`.
...
- **Iconos**: `assets/brand/` integrada — `tauri icon icon-1024.png` regeneró `src-tauri/icons/`; favicon.svg en desktop (`public/` + titlebar + index.html con título LIXBON) y favicons completos en `apps/web` (svg + png 16/32 + apple-touch).
