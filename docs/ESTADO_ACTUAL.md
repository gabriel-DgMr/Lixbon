...
- **Posicionamiento (las cuatro palancas)**:
  1. **Portada** `pages/LandingPage.jsx` en `/` para quien no tiene sesión (con sesión, `/` sigue siendo el chat; el chat público vive en `/chat`, noindex). H1, razones, cuatro productos, ejemplo de API, precios resumidos y FAQ con esquemas `Organization`, `WebSite` y `FAQPage`. Estilos en `styles/landing.css`.
