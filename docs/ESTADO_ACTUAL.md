...
- **Posicionamiento (las cuatro palancas)**:
  1. **Portada** `pages/LandingPage.jsx` siempre en `/` (el chat vive en `/chat`, noindex; con sesión los botones dicen «Ir al chat»). Minimalista, sin tarjetas: una columna de 1080 px, titulares a peso 500, secciones numeradas con reglas finas, productos en lista, precios sobre regla, FAQ en dos columnas. Dos ilustraciones de línea en `components/IlustracionesLanding.jsx` (clúster y recorrido del mensaje). **Tema claro/oscuro solo en la portada**: `data-tema` en `.landing` redefine los tokens (`styles/landing.css`), botón sol/luna en la barra (`PublicNav` acepta `extra`), preferencia en `localStorage` (`lixbon-tema-portada`); por defecto claro (fondo #F5F4EE). El resto de la app sigue oscura. H1, razones, cuatro productos, ejemplo de API, precios resumidos y FAQ con esquemas `Organization`, `WebSite` y `FAQPage`. Estilos en `styles/landing.css`.
