// docsContent.jsx — contenido de Folax Docs. Cada sección es un componente que
  ...
  return (
    <>
      ...
      <p className="docs__lead">
        FOLAX es una plataforma de chat con modelos de lenguaje que corre sobre un
...
function PrimerosPasos() {
  return (
    <>
      ...
      <h2>4. Lleva FOLAX a tu terminal</h2>
      <p>
...
function Cli({ base }) {
  return (
    <>
      <h1>CLI</h1>
      <p className="docs__lead">
        El CLI de FOLAX te deja chatear con el clúster desde la terminal, con modo
      ...
      <p>
        Esto descarga el CLI en <code>%USERPROFILE%\\.folax</code> y agrega el comando{' '}
        <code>folax</code> a tu PATH de usuario. Abre una terminal nueva para que el
      ...
      <p>
        Instala el CLI en <code>~/.folax</code> y crea el comando <code>folax</code>{' '}
        en <code>~/.local/bin</code>. Si <code>folax</code> no se reconoce, añade esa
        carpeta a tu PATH:
      </p>
      ...
      <table className="docs__table">
        ...
        <tbody>
          <tr><td><code>folax setup</code></td><td>Configuración inicial (servidor y API key)</td></tr>
          <tr><td><code>folax chat</code></td><td>Chat interactivo en la terminal</td></tr>
          <tr><td><code>folax status</code></td><td>Estado del gateway y del clúster</td></tr>
          <tr><td><code>folax update</code></td><td>Actualiza el CLI a la última versión</td></tr>
        </tbody>
      </table>
...
function Desktop() {
  return (
    <>
      ...
      <p className="docs__lead">
        La app de escritorio ofrece la experiencia completa de FOLAX como aplicación
...
function ApiDocs({ base }) {
  return (
    <>
      <h1>API</h1>
      <p className="docs__lead">
        FOLAX expone una API <strong>compatible con OpenAI</strong>. Si ya usas el SDK
      ...
      <CodeBlock code={`Authorization: Bearer folax_sk_tu_clave`} />

      <h2>Endpoint base</h2>
      <CodeBlock code={`${base}/v1`} />

      <h2>Listar modelos</h2>
      <CodeBlock label="cURL" code={`curl ${base}/v1/models -H "Authorization: Bearer folax_sk_tu_clave"`} />

      <h2>Chat completions (streaming)</h2>
      <CodeBlock
        label="cURL"
        code={`curl ${base}/v1/chat/completions \\
  -H "Authorization: Bearer folax_sk_tu_clave" \\
...
client = OpenAI(base_url="${base}/v1", api_key="folax_sk_tu_clave")
