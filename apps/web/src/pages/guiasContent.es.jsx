// guiasContent.es.jsx — artículos en español de /guides. Cada uno responde a
// una búsqueda concreta y solo cuenta lo que lixbon hace de verdad.
...
function ApiCompatible() {
  return (
    <>
      ...
      <p>
        Crea una cuenta, ve a <Link to="/account/profile">Ajustes → Cuenta → API keys</Link> y
        pulsa «Nueva key». Empieza por <code>lixbon_sk_</code> y solo se muestra una vez.
        Recarga unos créditos en <Link to="/account/billing">Facturación</Link>: la API se
  ...
  process.stdout.write(chunk.choices[0]?.delta?.content ?? "");
      ...
      <table className="docs__table">
        ...
        <tbody>
          ...
          <tr><td><code>403 model_not_priced</code></td><td>Ese modelo no tiene tarifa publicada</td><td>Usa uno de <Link to="/docs/api-pricing">la tabla</Link></td></tr>
      ...
      <p>
        ...
        los modelos están en <Link to="/docs/api-pricing">Precios de la API</Link>, y tu consumo
        diario en <Link to="/account/usage">Ajustes → Uso</Link>.
      </p>
    </>
  );
}
...
export LIXBON_API_KEY=lixbon_sk_tu_clave   # en Windows: set LIXBON_API_KEY=...`} />
      <p>
        La clave la creas en <Link to="/account/profile">Ajustes → Cuenta</Link>; los créditos,
        en <Link to="/account/billing">Facturación</Link>.
      </p>
...
print(f"{costo:.6f} USD")   # una respuesta normal ronda las 5 diezmilésimas`} />
      ...
      <ul>
        ...
        <li>Las tarifas vigentes están siempre en <Link to="/docs/api-pricing">Precios de la API</Link>.</li>
      </ul>
    </>
  );
}
  ...
  return (
    <>
      ...
      <ul>
        ...
        <li>Si desactivas «Historial de conversaciones» en <Link to="/account/privacy">Ajustes → Privacidad</Link>, los chats nuevos no se guardan; el uso sí se cuenta.</li>
      </ul>
      ...
      <p>
        Los detalles legales están en la <Link to="/legal/privacy">Política de privacidad</Link>.
      </p>
    </>
  );
}
