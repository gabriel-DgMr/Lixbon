// docsContent.es.jsx — contenido en español de lixbon Docs. Cada sección es un
// componente que recibe `base` (origen del gateway) para los ejemplos. El
// índice, los títulos y las descripciones viven en docsIndex.js (lo comparte
// el sitemap); docsContent.js junta este archivo con docsContent.en.jsx.
...
function Introduccion() {
  return (
    <>
      ...
      <ul>
        <li><strong><Link to="/docs/chat">Chat web</Link></strong> — la forma más rápida de empezar, sin instalar nada.</li>
        <li><strong><Link to="/docs/visuals">Visuals</Link></strong> — diseña landings, dashboards, emails, logos y prototipos navegables hablando con el modelo.</li>
        <li><strong><Link to="/docs/cli">CLI</Link></strong> — chat en la terminal con modo agente: lee y edita tu proyecto, ejecuta comandos y hace commits.</li>
        <li><strong><Link to="/docs/desktop">App de escritorio</Link></strong> — un IDE con autocompletado, chat y agente integrados.</li>
        <li><strong><Link to="/docs/remote">Remote</Link></strong> — sigue y dirige una sesión del agente desde el móvil.</li>
        <li><strong><Link to="/docs/api">API</Link></strong> — endpoints compatibles con OpenAI para tus propias aplicaciones.</li>
      </ul>

      <Callout>
        ¿Listo para empezar? Continúa con <Link to="/docs/getting-started">Primeros pasos</Link>.
      </Callout>
    </>
  );
}
...
function PrimerosPasos() {
  return (
    <>
      ...
      <p>
        Entra al <Link to="/chat">chat</Link> y regístrate con tu correo, nombre y una
      ...
      <p>
        En <Link to="/visuals">Visuals</Link> describes una web, un dashboard o un
        prototipo y el modelo lo construye en HTML; luego lo afinas hablando con él o
        tocando los elementos en el lienzo. Más en <Link to="/docs/visuals">Visuals</Link>.
      </p>

      <h2>5. Lleva lixbon a tu terminal o a tu editor</h2>
      <p>
        Cuando quieras usarlo mientras programas, instala el{' '}
        <Link to="/docs/cli">CLI</Link> o la <Link to="/docs/desktop">app de escritorio</Link>.
...
function ChatWeb() {
  return (
    <>
      <h1>Chat web</h1>
      <p className="docs__lead">
        El chat de <Link to="/chat">lixbon.com</Link> es la puerta de entrada: conversaciones con
      ...
      <p>
        ...
        <Link to="/account/privacy">Ajustes → Privacidad</Link>, los chats nuevos no se guardan.
      </p>
      ...
      <p>
        ...
        cómo <Link to="/plans">mejorar el plan</Link>.
      </p>
    </>
  );
}
  ...
  return (
    <>
      ...
      <p>
        ...
        <code>lixbon_sk_…</code> (la generas en <Link to="/account/profile">Ajustes → Cuenta</Link>).
...
function Desktop() {
  return (
    <>
      ...
      <h2>Instalación</h2>
      <p>
        Descarga el instalador desde <Link to="/apps">Aplicaciones</Link> y
      ...
      <ul>
        ...
        <li><strong>Remote</strong>: sigue la sesión desde el móvil (<Link to="/docs/remote">ver Remote</Link>).</li>
      </ul>
      ...
      <p>
        ...
        historial de versiones está en <Link to="/news">Novedades</Link>.
      </p>

      <h2>App de Android</h2>
      <p>
        En <Link to="/apps">Aplicaciones</Link> también está la app de Android
      ...
      <Callout>
        ...
        una versión; mientras tanto puedes usar el <Link to="/docs/cli">CLI</Link> o el{' '}
        <Link to="/chat">chat web</Link>.
      </Callout>
    </>
  );
}
...
function Remote() {
  return (
    <>
      ...
      <p>
        Con sesión iniciada, <Link to="/remote">lixbon.com/remote</Link> lista tus sesiones
        activas.
      </p>
    </>
  );
...
function ApiDocs({ base }) {
  return (
    <>
      ...
      <h2>Autenticación</h2>
      <p>
        Genera una API key en <Link to="/account/profile">Ajustes → Cuenta</Link>. Se muestra
    ...
    print(chunk.choices[0].delta.content or "", end="")`}
      ...
      <table className="docs__table">
        ...
        <tbody>
          ...
          <tr><td><code>402 insufficient_credits</code></td><td>Sin saldo de créditos: recarga en <Link to="/account/billing">Facturación</Link>.</td></tr>
      ...
      <Callout>
        ...
        que consumas: consulta <Link to="/docs/api-pricing">Precios de la API</Link> y las
        recetas de integración en <Link to="/docs/using-your-api-key">Usar tu API key</Link>.
      </Callout>
    </>
  );
}
...
function UsarApiKey({ base }) {
  return (
    <>
      ...
      <h2>1. Crea tu key</h2>
      <p>
        Ve a <Link to="/account/profile">Ajustes → Cuenta → API keys</Link> y pulsa
        ...
        tu <Link to="/docs/plans">plan</Link>.
      </p>

      <h2>2. Recarga créditos</h2>
      <p>
        Las peticiones con API key se pagan con <strong>créditos prepago</strong> según los
        tokens que uses (ver <Link to="/docs/api-pricing">Precios de la API</Link>). Recarga
        saldo en <Link to="/account/billing">Ajustes → Facturación</Link>. Sin saldo, la
  ...
  - name: lixbon
    ...
    apiKey: lixbon_sk_tu_clave`}
      ...
      <Callout>
        ...
        <Link to="/account/usage">Ajustes → Uso</Link>, y el costo por modelo en{' '}
        <Link to="/docs/api-pricing">Precios de la API</Link>.
      </Callout>
    </>
  );
}
...
function PreciosApi() {
  ...
  return (
    <>
      ...
      <ul>
        <li>Se compran en <Link to="/account/billing">Ajustes → Facturación</Link> con tarjeta, en un pago único.</li>
        ...
        <li>Con la <strong>recarga automática</strong> el pack que elijas se cobra solo cuando el saldo baja de $5 (ver <Link to="/docs/billing">Facturación</Link>).</li>
      </ul>
      ...
      <Callout>
        ...
        <Link to="/account/usage">Ajustes → Uso</Link>.
      </Callout>
    </>
  );
}
  ...
  return (
    <>
      ...
      <p>
        ...
        vigentes y el cambio de plan están en la página de <Link to="/plans">Planes</Link>.
      </p>
      ...
      <ul>
        ...
        <li>Puedes ver tu consumo en tiempo real en <Link to="/account/usage">Ajustes → Uso</Link>.</li>
      </ul>
      ...
      <p>
        ...
        hoy solo se cobra la diferencia prorrateada (ver <Link to="/docs/billing">Facturación</Link>).
      </p>

      <h2>¿Y la API?</h2>
      <p>
        Las peticiones con API key <strong>no</strong> consumen la cuota del plan: se pagan
        con <Link to="/docs/api-pricing">créditos prepago</Link> por tokens y pueden usar
...
function Facturacion() {
  return (
    <>
      ...
      <p className="docs__lead">
        Todo se gestiona en <Link to="/account/billing">Ajustes → Facturación</Link>: el
      ...
      <ol>
        <li>En <Link to="/plans">Planes</Link> elige Pro o Advance. Se abre el pago sin salir de lixbon.</li>
      ...
      <Callout>
        Las condiciones completas están en <Link to="/legal/refunds">Cancelaciones y
        reembolsos</Link> y en los <Link to="/legal/terms">Términos</Link>.
      </Callout>
    </>
  );
}
  ...
  return (
    <>
      ...
      <p className="docs__lead">
        ...
        <Link to="/account/privacy">Ajustes → Privacidad</Link>.
      </p>
      ...
      <Callout>
        El texto completo está en la <Link to="/legal/privacy">Política de privacidad y
        tratamiento de datos</Link>; las condiciones del servicio, en los{' '}
...
const CUERPOS = {
  ...
  remote: Remote,
  api: ApiDocs,
  'using-your-api-key': UsarApiKey,
  'api-pricing': PreciosApi,
  plans: Planes,
  billing: Facturacion,
  privacy: Privacidad,
};
