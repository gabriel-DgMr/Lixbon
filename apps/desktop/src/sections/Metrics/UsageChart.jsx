// UsageChart.jsx — barras de tokens/día (30 días). Portado de apps/web.
// Una sola serie: color tinta uniforme, barras finas, tooltip por barra,
// labels selectivos (solo el máximo).
...
export function UsageChart({ daily }) {
  ...
  const data = useMemo(() => {
    ...
    for (const row of daily) {
      const key = row.usage_date || row.date;
      byDate[key] = (byDate[key] || 0) + (row.total_tokens || 0);
    }
