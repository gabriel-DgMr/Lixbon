...
def hash_password(password: str) -> str:
    """Hash seguro con scrypt + salt. Nuevo formato para v2.0."""
    import os
    salt = os.urandom(16)
...
def _verify_password_internal(password: str, stored_hash: str) -> bool:
    """Verifica password contra hash scrypt o SHA-256 legacy."""
...
def init_db() -> None:
    ...
    with get_conn() as conn:
        ...
        conn.executescript("""
            ...
            -- Índices para mejorar velocidad en consultas frecuentes (v2.0)
            CREATE INDEX IF NOT EXISTS idx_messages_conv
                ON messages(conversation_id, created_at);
            CREATE INDEX IF NOT EXISTS idx_conversations_user
                ON conversations(user_id, updated_at);
            CREATE INDEX IF NOT EXISTS idx_task_emb_user_date
...
def verify_user(username: str, password: str) -> dict[str, Any] | None:
    """
    Verifica usuario/contraseña. Soporta hashes scrypt (v2.0) y SHA-256 (legacy).
    Si el hash es legacy y la contraseña es correcta, lo migra automáticamente a scrypt.
    """
    with get_conn() as conn:
        row = conn.execute(
        ...
        if not _verify_password_internal(password, row["password_hash"]):
            return None
        # Migrar hash legacy SHA-256 → scrypt si es necesario
...
def list_audit_events(
    ...
    """Devuelve los últimos eventos del audit log (opcionalmente filtrado por usuario)."""
    with get_conn() as conn:
        if user_id is not None:
            rows = conn.execute(
                """
...
# Funciones de soporte para Gestión de Versiones (app_versions) en SQLite
def get_all_versions() -> list[dict[str, Any]]:
    with get_conn() as conn:
        rows = conn.execute("SELECT id, version, channel, release_date, title, changelog_json, download_url, checksum_sha256, created_at FROM app_versions ORDER BY release_date DESC").fetchall()
    result = []
    for row in rows:
        r = dict(row)
        try:
            r["changelog"] = _json.loads(r.pop("changelog_json"))
        except Exception:
            r["changelog"] = []
        result.append(r)
    return result
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            ON CONFLICT(version) DO UPDATE SET
                channel = excluded.channel, release_date = excluded.release_date,
                title = excluded.title, changelog_json = excluded.changelog_json,
                download_url = excluded.download_url, checksum_sha256 = excluded.checksum_sha256
            """,
...
# Funciones de soporte para Métricas (token_usage_daily) en SQLite
