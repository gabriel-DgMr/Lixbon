...
1. Toda migración se aplica primero en `folax-staging`.
2. Backup manual de prod ANTES de aplicar la misma migración en prod.
3. Nunca ejecutar `reset_staging.py` con la URL de prod (el script se niega si `FOLAX_ENV=production`, pero no te confíes solo de eso).
