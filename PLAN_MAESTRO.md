# FOLAX — Plan Maestro: de proyecto LAN a servicio SaaS en producción
...
- **Producción**: Postgres gestionado de Railway (BD `folax-prod`).
- **Desarrollo local**: el gateway corriendo en tu PC se conecta a una **BD de staging en Railway** (`folax-staging`). **No corre ninguna base de datos en tu PC** — ni Postgres local, ni Docker, ni SQLite.
...
| **API key** (`folax_sk_...`) | CLI, desktop, integraciones | Larga, revocable | **Solo hash** — se muestra una única vez |
...
| 1.2b | Crear en Railway **dos Postgres**: `folax-staging` (para depurar desde tu PC) y `folax-prod`. El `.env` local apunta a staging vía `DATABASE_URL`; **nada de BD corriendo en tu PC** |
...
| Migración de BD rompe datos | Medio | Alembic + backup previo + toda migración se prueba primero en `folax-staging` antes de aplicarse a prod |
