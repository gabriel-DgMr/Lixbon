...
if exist ".venv\Scripts\python.exe" (
    start "FOLAX-Gateway" /min cmd /k "cd /d "%~dp0" && .venv\Scripts\python.exe -m uvicorn app.main:app --host 0.0.0.0 --port 8000 2>&1 | tee logs\gateway.log"
) else (
    start "FOLAX-Gateway" /min cmd /k "cd /d "%~dp0" && uvicorn app.main:app --host 0.0.0.0 --port 8000 2>&1 | tee logs\gateway.log"
)
goto :EOF
